...
function guestPage(sessionName) {
...
document.getElementById('q').addEventListener('input',e=>{
  ...
  t=setTimeout(async()=>{
    const r=await fetch('/api/search?q='+encodeURIComponent(v));const j=await r.json();
...
async function load(){
  const r=await fetch('/api/top');const j=await r.json();
...
class GuestServer {
  ...
  start(opts) {
    ...
    this.token = opts.token || Math.random().toString(36).slice(2, 8);
    ...
    this.server = http.createServer((req, res) => {
      ...
      res.end(guestPage(self.sessionName));
    });
